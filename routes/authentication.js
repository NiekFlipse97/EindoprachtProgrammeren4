...
function login(email, password, hash, callback) {
    ...
    if (password !== hash) {
        ...
        genereteTokenAfterLogin(email, password, callback);
    }
}

function genereteTokenAfterLogin(email, password, callback) {
...
router.route("/register").post((request, response) => {
    ...
    bcrypt.genSalt(10, function (err, salt) {
        bcrypt.hash(password, salt, function (err, hash) {
            ...
            db.query(`SELECT * FROM user WHERE Voornaam = "${firstName}" AND Achternaam = "${lastName}" AND Email = "${email}" AND Password = "${hash}"`, (error, rows, fields) => {
                if (rows.length > 0) {
                    // User already exists.
                    ...
                    db.query(query, (error, rows, fields) => {
                        ...
                        if (!error) {
                            login(email, password, hash, (error, result) => {
...
router.route("/login").post((request, response) => {
    ...
    db.query("SELECT Password FROM user WHERE Email = ?", [email], (error, rows, fields) => {
        ...
        // Check in database for matching username and password.
        login(email, password, rows[0].Password, (error, result) => {
            if (error) response.status(error.code || 500).json(error);
            else response.status(200).json(result);
        });
    });
